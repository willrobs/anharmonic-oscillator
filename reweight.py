...
def reweighting(spring_const,mass,la,lattice_spacing,num_config,num_lat_points,mass_prime,spring_const_prime,lap,corrt,bootstraps):
    #### mu and mup refer to their squares 
    mu=spring_const
    m=mass
    a=lattice_spacing
    niter=num_config
    ...
    smp=str(smp)
    slap=str(slap)
    namep=smup+slap+smp+sma+sniter+spoints
    ...
    for g in range(num_bins):
        ...
        for j in range(bin_size):
            ...
            t3=c3p*ssps.x4[j + g*bin_size]
            sum2=sum2+np.exp(t1 + t2 + t3)
    ...
    for m in range(corrt):
        ...
        for h in range(num_bins):
            ...
            for n in range(bin_size):
                ...
                sum1=sum1+(x0*xn)*np.exp(t1 + t2 + t3)
                sum2=sum2+(x0*xnm1)*np.exp(t1 + t2 + t3)
