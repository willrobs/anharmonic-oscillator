...
def corr(spring_const,mass,la,lattice_spacing,num_config,num_lat_points):
    mu=spring_const
    m=mass
    a=lattice_spacing
    niter=num_config
...
def binning(spring_const,mass,la,lattice_spacing,num_config,num_lat_points,size_of_bin,raw_data):
    mu=spring_const
    m=mass
    a=lattice_spacing
    niter=num_config
    ...
    for n in range(num_bins):
        ...
        f.write('\n' + str(sum1/(points*bin_size)) + '\t' + str(sum2/(points*bin_size)))
...
array=corr(mu,mass,la,a,num_config,num_lat_points)
binning(mu,mass,la,a,num_config,num_lat_points,array[1],array[0])
