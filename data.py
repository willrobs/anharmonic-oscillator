...
def data(spring_const,mass,la,lattice_spacing,num_config,num_lat_points):
    mu=spring_const
    m=mass
    a=lattice_spacing
    niter=num_config
    ...
    for m in range(ttime): #### thermalizaing state
        ...
        state=metro(state,points,0,0,0) #### config,num_lat_points
    sum1=0
    sum2=0
    sum3=0
    ...
    for m in range(niter):
        ...
        array=metro(state,points,delx2,x2,x4) #### config,num_lat_points
        state=array[0]
        delx2=array[1]
        x2=array[2]
        x4=array[3]
        for h in range(int(points)):
            f.write(str(state[h]) + '\n') 
        g.write('\n' + str(delx2) + '\t' + str(x2) + '\t' + str(x4))
        
    f.close()
    g.close()
    print("Raw data gathered.")
    
def metro(config,num_lat_points,delx2,x2,x4):
    
    state=config
    points=num_lat_points
    if delx2 ==0 and x2 ==0 and x4==0:
        skip=True
    else:
        skip=False
...
def daction(x1,x,xm1,xp):
    ...
    delta_s=dxn*(sxn*(c1 + c2 + c3*sxn2) -sx1*c1)
...
data(mu,m,la,a,num_config,num_lat_points)
